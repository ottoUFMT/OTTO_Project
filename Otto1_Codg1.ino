...
unsigned long duration; //duração da gravação
int tempo;              //auxiliar para duração da gravação
int GravAux=0;          //1 enquanto a gravação estiver tocando
...
int pino_rec = 34;  //pino gravar
int pino_playe = 36; //pino toca gravação
...
int state;            //usado na gravação
int estadograva=0;    //gravando ou não
...
void loop()
{
  ...
  if( estadoliga==1){
    ...
    {  
      ...
      tempo=millis()-duration;
      Serial.println("parando gravação  ");
      digitalWrite(pino_rec, LOW);
      estadograva=0;
      }
    }  
   if (armazenavalor == 0xFFE21D) //Verifica se a tecla 3 foi acionada  
    {  
      if(GravAux==0){
      GravAux=1;
      duration=millis();
